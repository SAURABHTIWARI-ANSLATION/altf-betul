  ...
  return (
    <div className="flex items-center gap-3 min-w-0">
      ...
      <div className="relative shrink-0 w-16 h-9 rounded-lg overflow-hidden border border-gray-100 bg-gray-50">
        {hasImage ? (
          ...
          <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-indigo-50 to-indigo-100">
            <span className="text-indigo-400 font-bold text-sm">{video.name?.[0]?.toUpperCase()}</span>
          </div>
        )}
      ...
      <div className="min-w-0">
        <span className="font-semibold text-gray-900 text-sm truncate block leading-tight">{video.name}</span>
        {video.subCategory && (
          <span className="text-[11px] text-indigo-500 font-medium mt-0.5 block truncate capitalize">{video.subCategory}</span>
        )}
      </div>
    </div>
  );
      ...
      <div className="flex items-center gap-3 px-5 py-3 bg-white border-b border-gray-100 shrink-0">
        ...
        <span className="text-xs text-gray-400 shrink-0">
          {searchQuery && filteredCount !== totalCount
            ...
            : <><span className="font-semibold text-gray-600">{totalCount}</span> video{totalCount !== 1 ? "s" : ""}</>}
        </span>
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-12 gap-6">
                            ...
                            <div className="sm:col-span-8 space-y-5">
                              <div className="flex items-start justify-between gap-3">
                                <div>
                                  <h3 className="font-bold text-gray-900 text-base leading-tight">{video.name}</h3>
                                </div>
