...
import {
  ...
  ExternalLink, Tag, Folder, VideoIcon, Play, Clock,
} from "lucide-react";
...
function SortIcon({ sorted }) {
  if (sorted === "asc") return <ChevronUp className="w-3.5 h-3.5 text-violet-500" />;
  if (sorted === "desc") return <ChevronDown className="w-3.5 h-3.5 text-violet-500" />;
...
function ResizeHandle({ header }) {
  return (
    <div onMouseDown={header.getResizeHandler()} onTouchStart={header.getResizeHandler()}
      ...
      <div className={`w-0.5 h-5 rounded-full transition-colors ${header.column.getIsResizing() ? "bg-violet-500" : "bg-gray-300 group-hover/resize:bg-violet-400"}`} />
    </div>
  );
}
        ...
        {table.getAllLeafColumns().map((col) => {
          ...
          return (
            <label key={col.id} className="flex items-center gap-2.5 px-2 py-1.5 rounded-lg hover:bg-gray-50 cursor-pointer">
              <input type="checkbox" checked={col.getIsVisible()} onChange={col.getToggleVisibilityHandler()} className="w-3.5 h-3.5 accent-violet-500 cursor-pointer" />
...
function CopyButton({ text }) {
  ...
  return (
    <Tooltip label={copied ? "Copied!" : "Copy URL"}>
      <button onClick={copy}
        className={`p-1.5 rounded-md transition ${copied ? "text-green-500 bg-green-50" : "text-gray-400 hover:text-violet-500 hover:bg-violet-50"}`}>
...
/* ── Video Preview Cell (hover to play) ── */
function VideoPreviewCell({ url, title }) {
  ...
  return (
    <div
      className="w-16 h-12 rounded-lg overflow-hidden bg-gray-100 border border-gray-200 shrink-0 relative cursor-pointer"
          ...
          {!hovering && (
            <div className="absolute inset-0 flex items-center justify-center bg-black/20">
              <div className="w-5 h-5 rounded-full bg-white/80 flex items-center justify-center">
                <Play className="w-2.5 h-2.5 text-gray-800 ml-0.5" fill="currentColor" />
              </div>
            </div>
          )}
        </>
...
export default function VideoTable({
  ...
  loading,
  tabs = ["All",  "Marketing", "Tutorials", "Testimonials", "Product", "Social", "Blogs", "Other"],
  activeTab,
  onTabChange,
  search,
  onSearch,
  ...
  const toggleAll = () => allSelected ? setSelectedVideos([]) : setSelectedVideos(videos.map((v) => v.id));
  ...
  const columns = useMemo(() => [
    {
      ...
      header: () => (
        <input type="checkbox" checked={allSelected}
          ...
          onChange={toggleAll}
          className="w-4 h-4 rounded border-gray-300 cursor-pointer accent-violet-500" />
      ),
      cell: ({ row }) => (
        <input type="checkbox" checked={selectedVideos.includes(row.original.id)}
    ...
    },
    {
      id: "preview",
      header: "Preview",
      size: 90, minSize: 90, maxSize: 90, enableSorting: false, enableResizing: false,
      cell: ({ row }) => (
        <VideoPreviewCell url={row.original.url} title={row.original.title} />
      ),
    },
    {
      accessorKey: "title",
    ...
    },
    {
      ...
      cell: ({ getValue }) => getValue()
        ? <span className="inline-flex items-center gap-1 text-xs text-violet-600 bg-violet-50 px-2 py-0.5 rounded-full font-medium"><Folder className="w-3 h-3" />{getValue()}</span>
    ...
    },
    {
      ...
      cell: ({ getValue }) => {
        const v = getValue();
        return v
          ? <span className="inline-flex items-center gap-1 text-gray-500 text-xs font-mono"><Clock className="w-3 h-3" />{v}</span>
          : <span className="text-gray-400">—</span>;
      },
    },
    {
    ...
    },
    {
      ...
      cell: ({ getValue }) => {
        const v = getValue() || "";
        const ext = v.split("/")[1]?.toUpperCase().replace("QUICKTIME", "MOV") || "—";
        ...
        <div className="flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
          ...
          <Tooltip label="Open in new tab">
            <a href={row.original.url} target="_blank" rel="noreferrer"
              className="p-1.5 rounded-md text-gray-400 hover:text-violet-500 hover:bg-violet-50 transition">
  ...
  ], [videos, selectedVideos, allSelected, someSelected]); // eslint-disable-line
  ...
  return (
    <div className={wrapperClass}>
          ...
          <div className="flex flex-wrap gap-1.5">
            {tabs.map((tab) => (
              <button key={tab} onClick={() => onTabChange?.(tab)}
                className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${activeTab === tab ? "bg-violet-600 text-white shadow-sm" : "bg-white border border-gray-200 text-gray-600 hover:bg-gray-100"}`}>
                {tab}
              </button>
            ))}
          </div>
            ...
            <div className="relative" ref={columnPanelRef}>
              <Tooltip label="Toggle columns" direction="bottom">
                <button onClick={() => setShowColumnPanel((v) => !v)}
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition border ${showColumnPanel ? "bg-violet-50 border-violet-200 text-violet-600" : "bg-white border-gray-200 text-gray-600 hover:bg-gray-100"}`}>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="relative w-full sm:w-72">
            ...
            <input value={search ?? ""} onChange={(e) => onSearch?.(e.target.value)}
              ...
              className="w-full pl-8 pr-8 py-1.5 text-sm bg-white border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-violet-400/40 focus:border-violet-400 placeholder:text-gray-400 transition" />
            {search && (
              ...
              table.getRowModel().rows.map((row) => {
                ...
                return (
                  <tr key={row.id} onClick={() => toggleRow(row.original.id)}
                    className={`cursor-pointer transition-colors ${isSelected ? "bg-violet-50 hover:bg-violet-100" : "hover:bg-gray-50"}`}>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 px-5 py-3 bg-gray-50 border-t border-gray-100 shrink-0">
        <p className="text-xs text-gray-500">{rowCount === 0 ? "No results" : `Showing ${startRow}–${endRow} of ${rowCount} videos`}</p>
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            <span className="text-xs text-gray-500">Rows per page</span>
            <select value={pageSize} onChange={(e) => onPaginationModelChange({ page: 0, pageSize: Number(e.target.value) })}
              className="text-xs border border-gray-200 rounded-md px-2 py-1 bg-white text-gray-700 focus:outline-none focus:ring-1 focus:ring-violet-400">
