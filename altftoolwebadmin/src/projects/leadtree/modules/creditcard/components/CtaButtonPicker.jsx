...
import { Zap, X, Copy, Check, ExternalLink, ChevronDown, ChevronUp, Smile } from "lucide-react";
...
//   { id: "CTA_GLASS",     label: "Glass",     defaultIcon: "🧊", defaultText: "Explore Now",        desc: "Frosted glass, modern UI" },
];
...
function LivePreview({ variantId, icon, text }) {
  return (
    <div className="CTA_CK_BANNER" style={{ margin: 0 }}>
      <a href="#" className={`CTA_CK ${variantId}`} onClick={(e) => e.preventDefault()}>
        {icon.trim() && <span className="CTA_ICON">{icon.trim()}</span>}
...
/* ─────────────────────────────────────────────
   VARIANT CARD
   iconOverride:
     null  → use variant's own defaultIcon
     ""    → no icon
     "🎯"  → custom icon for all variants
───────────────────────────────────────────── */
function VariantCard({ variant, href, customText, iconOverride }) {
  const [expanded, setExpanded] = useState(false);

  const icon = iconOverride !== null ? iconOverride : variant.defaultIcon;
...
export default function CtaButtonPicker({ trigger }) {
  ...
  // str  → custom icon applied to all variants
  ...
  const handleNoIcon = () => {
    setIconInput("");
    setIconOverride("");       // explicit empty string = strip icon
  };

  const handleDefaultIcons = () => {
    setIconInput("");
    setIconOverride(null);     // null = back to per-variant defaults
  };
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 shrink-0">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h2 className="text-sm font-bold text-gray-900">CTA Button Picker</h2>
                  <p className="text-[10px] text-gray-400">Copy HTML → paste in CKEditor's Insert HTML</p>
                </div>
              </div>
              <button
                type="button"
            ...
            <div className="px-6 py-4 border-b border-gray-100 bg-gray-50 shrink-0 space-y-3">
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                ...
                <div className="flex-1 space-y-1">
                  <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">
                    Button Label <span className="font-normal normal-case">(optional)</span>
                  </label>
                  <input
                    type="text"
                    value={customText}
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <input
                    ...
                    className={`flex-1 min-w-[160px] text-sm px-3 py-2 rounded-xl border bg-white focus:outline-none focus:ring-2 focus:ring-blue-400/30 focus:border-blue-400 transition
                      ${customIconActive ? "border-blue-400 ring-2 ring-blue-400/20" : "border-gray-200"}`}
                  />
                ...
                <p className="text-[10px] text-gray-400 h-3">
                  {noIconActive    && "Icon removed — all buttons will render text-only."}
                  {defaultIconActive && "Using each variant's built-in default icon."}
                  {customIconActive  && `"${iconInput}" applied to all variants.`}
                </p>
              </div>
            </div>
            ...
            <div className="px-6 py-2 bg-blue-50 border-b border-blue-100 shrink-0">
              <p className="text-[10px] text-blue-600 font-medium">
                💡 Pick a style → click <strong>Copy HTML</strong> → in CKEditor click{" "}
                <code className="bg-blue-100 px-1 rounded">{"<>"} HTML Embed</code> or use{" "}
                <code className="bg-blue-100 px-1 rounded">Insert → HTML</code> and paste.
              </p>
            </div>
            ...
            <div className="px-6 py-3 border-t border-gray-100 bg-gray-50 shrink-0">
              <p className="text-[10px] text-gray-400 text-center">
                Styles are defined in <code className="bg-gray-100 px-1 rounded">cta-buttons.css</code> — make sure it's loaded on your public blog pages.
              </p>
            </div>
