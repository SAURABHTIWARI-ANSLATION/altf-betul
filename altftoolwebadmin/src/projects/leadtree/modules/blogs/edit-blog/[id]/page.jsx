    ...
    (async () => {
      try {
        ...
        if (!data) {
          ...
          router.push("/leadtree/blogs"); return;
        }
        setFormData({
  ...
  const updateBlogHandler = async (status) => {
    ...
    try {
      ...
      logAuditEvent({
        ...
        route: `/leadtree/blogs/edit-blog/${id}`,
      });

      setUploadStep("done");
      emitAlert({ type: "success", message: status === "published" ? "Blog published!" : "Draft saved." });
      setTimeout(() => router.push("/leadtree/blogs"), 600);
    } catch (err) {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <div className="space-y-5">
            ...
            {formData.status === "published" && (
              <button onClick={() => router.push(`/leadtree/blogs/view-blogs/${id}`)}
